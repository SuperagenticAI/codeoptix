...
class ReflectionGenerator:
    ...
    def generate(
        ...
        for behavior_name, behavior_data in behaviors.items():
            ...
            severity = behavior_data.get("scenario_results", [{}])[0].get("behavior_result", {}).get("severity", "medium")
    ...
    def _identify_root_causes(self, behavior_data: Dict[str, Any]) -> List[str]:
        ...
        for scenario_result in scenario_results:
            ...
            if eval_results.get("static_analysis", {}).get("bandit"):
                ...
                if bandit_data and bandit_data.get("metrics", {}).get("_totals", {}).get("SEVERITY.HIGH", 0) > 0:
    ...
    def _generate_recommendations(
        ...
        if len(evidence) > 5:
            recommendations.append("Multiple issues detected - consider comprehensive prompt revision")
    ...
    def _generate_overall_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        failed_behaviors = [name for name, data in behaviors.items() if data.get("score", 0.0) < 0.7]
        if failed_behaviors:
