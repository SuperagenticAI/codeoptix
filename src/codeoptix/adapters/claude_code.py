"""Claude Code adapter for CodeFlect.
...
provides a simplified interface for CodeFlect's evaluation framework using the
...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if provider != LLMProvider.OLLAMA:
            api_key = llm_config.get("api_key") or os.getenv(f"{provider_name}_API_KEY") or os.getenv("ANTHROPIC_API_KEY")
        ...
        if provider == LLMProvider.OLLAMA:
            ...
            # Use current Claude Code default model (claude-sonnet-4-5-20250929 or claude-3-5-sonnet-20241022)
...
- Write clear, maintainable code"""
    ...
    def _extract_code_and_tests(self, response: str) -> tuple[str, Optional[str]]:
        """
