"""Gemini CLI adapter for CodeFlect.
...
class GeminiCLIAdapter(AgentAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_key = llm_config.get("api_key") or os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
...
- Follow the user's requirements"""
    ...
    def _execute_gemini_cli(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"Gemini CLI exited with code {process.returncode}: {stderr}"
                )
    ...
    def _extract_from_json(self, result: Dict[str, Any]) -> tuple[str, Optional[str]]:
        """
