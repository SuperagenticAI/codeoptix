"""Codex (OpenAI) adapter for CodeFlect.
...
provides a simplified interface for CodeFlect's evaluation framework.
"""
...
class CodexAdapter(AgentAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_key = llm_config.get("api_key") or os.getenv("OPENAI_API_KEY") or os.getenv("CODEX_API_KEY")
...
- Follow specifications exactly"""
    ...
    def _execute_codex_cli(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"Codex CLI exited with code {process.returncode}: {stderr}"
                )
    ...
    def _extract_from_jsonl(self, result: Dict[str, Any]) -> tuple[str, Optional[str]]:
        """
