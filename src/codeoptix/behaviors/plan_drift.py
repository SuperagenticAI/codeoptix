...
class PlanDriftBehavior(BehaviorSpec):
    ...
    def evaluate(
                ...
                if func_name in code:
                    for param in params:
                        if param not in code:
                            issues_found += 1
                            evidence.append(f"Required parameter '{param}' missing in '{func_name}'")
        ...
        if issues_found == 0:
            ...
            total_checks = max(1, len(plan_keywords) if plan else 0 + len(requirements) + len(api_spec.get("functions", [])))
