...
class TestEvolutionEngine:
    ...
    def test_evolution_engine_with_config(
        self, mock_adapter, mock_llm_client, temp_artifacts_dir
    ):
    ...
    def test_evolve_returns_results(
        self, mock_adapter, mock_llm_client, temp_artifacts_dir,
        sample_evaluation_results, sample_reflection_content
    ):
        """Test that evolve returns results."""
        eval_engine = EvaluationEngine(mock_adapter, mock_llm_client, config={
            "scenario_generator": {"num_scenarios": 1, "use_bloom": False}
        })
        artifact_manager = ArtifactManager(artifacts_dir=temp_artifacts_dir)
    ...
    def test_evolve_saves_artifacts(
        ...
        evolved_path = artifact_manager.get_evolved_prompts_path(
            sample_evaluation_results["run_id"]
        )
        # Note: File might not exist if save failed, but structure should be correct
        assert "prompts" in evolved
